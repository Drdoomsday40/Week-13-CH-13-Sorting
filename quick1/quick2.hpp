...
static void partition2(int data[], size_t n, size_t& pivot_index)
...
{
    if ((data[0] < data[n] && data[n] < data[n / 2]) || (data[n / 2] < data[n] && data[n] < data[0]))
        swap(data[0], data[n]);
    else if ((data[n] > data[0] && data[0] < data[n / 2]) || (data[n / 2] < data[0] && data[0] < data[n]))
        swap(data[0], data[0]);
    else
...
void quicksort2(int data[], size_t n)
// Library facilities used: stdlib.h
{
    ...
    if (n > 1) {
        partition2(data, n, pivot_index);
        n1 = pivot_index;
        n2 = n - n1 - 1;
        quicksort2(data, n1);
        quicksort2((data + pivot_index + 1), n2);
    }
}
